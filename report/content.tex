...
%expliquer brièvment les différentes sections qui vont etre abordée dans ce qui suit
...
%% TODO dire que toutes les protections recommandées pour un site de production ne sont pas configurée. Il aurait fallu configurer htaccess et php.ini pour faire cela et configurer encore d'autres fichiers coté serveur Mentioner egalement que les redirections n'ont pas été personalisées et ou configurée. Dans le cadre d'une mise en production cela doit être fait
...
%tourne sur la version 0.12.2 sortie en May 2017. La version stable est sortie le 26 AVRIL 2018 avec pas mal de changements
%dire que pour interagir avec la base de données j'ai utlisé le module MysqlDB. Ce module permet d'écrire les requetes en pure sql. J'ai choisi ce module plutot qu'un Object Relational Mapper (ORM) comme flask.sqlachemy pour deux raison la première étant que Mysqldb permet d'écrire les requêtes en pure SQL ce que sqlachemy ne fait pas vu qu'il permet d'abstraire cette partie ce qui peut etre une bonne chose (en terme d'optimization de gestion de curseur,etc) mais aussi une mauvaise chose car on ne comprends pas necessairement ce qui se passe derrière. La deuxième raison est que la mise en place du ORM n'est pas aussi simple que cela. Cela se comprend puisque on abstrait bcp de choses. J'ai donc fait le choix du module MysqlDb plutot que le choix d'un orm. Cependant pour une mise en production il est recommandé d'utiliser un ORM plutot que d'utiliser un module où il faut écrire toutes les requetes SQL car un ORM permet de passer d'un type de base de donnée à l'autre, il permet également d'être maintenu par des personnes ne maitrisant pas nécessairement le language SQL et de manière générale est plus facile à maintenir.
\subsection{functionalities implemented}
Several functionalities have been implemented.
%% expliquer que pour chaque méthode implementée coté serveur les paramètres envoyés sont verifiés parsé. Si tous les paramètres sont correctes on interroge la base de donnée en fonction  et on retorune le results. Le framework flask permet dejà de faire une première vérification car dans les routes que l'on spécifie on peut lui indiquer des types prédefini <int, string etc. Si les types donnés ne respectes pas ses restrictions l'utilisateur recevra une page 404 car flask interprete cela comme une route différente vu que les contraintes ne sont pas les meme 
...
URI template des requetes pourrait être uniformisé pour respecter au mieux les requetes.
